const { captureException } = require("@middleware.io/agent-apm-nextjs");
...
export async function GET(request) {
  ...
  try {
    ...
    if (isNaN(a) || isNaN(b)) {
      ...
      return new Response(
        JSON.stringify({ error: "Query parameters must be numbers" }),
        {
          status: 400,
          headers: { "Content-Type": "application/json" },
        }
...
function divide(a, b) {
  ...
  if (a < b) throw new Error("a must be greater than or equal to b");
